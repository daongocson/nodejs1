...
const getKskService = async (idoa,today) => {
    if(idoa){
        // check xem bảng lấy số chưa, nếu có rồi trả về stt
        let sqlServer = "select * FROM [His_xml].[dbo].[tb_ksk] where ngaylog>CAST( GETDATE() AS Date) and idoa="+idoa+" order by ngaylog desc";          
        await sql.connect(sqlConfig);           
        let result= await sql.query(sqlServer);    
        var rows = result.recordset;
        if(rows.length>0){
...
const postDkKskService = async (dataksk) => {
    const {idoa,tenbn,phonenum,idbs,tenbs,namsinh,addbn}=dataTaxi;
    try {             
...
const getDsKskService = async function(){
    try {
        var datetime = new Date();   
        await sql.connect(sqlConfig);   ;
        let strSql = "select TOP 30 * FROM [His_xml].[dbo].[tb_ksk] where trangthai=1 order by ngaylog desc";        
        let result= await sql.query(strSql);  
        return result.recordset;
    } catch (error) {
        console.log(error);
        ...
        rows.forEach(function(item) {  
         ...
         rowsInfo.forEach(function(item) {                 
            ...
            if(item.dm_gioitinhid==1)
                arrayInfo.push({id:"11",name:"Giới tính",gender:"Nam"});
            else
                arrayInfo.push({id:"11",name:"Giới tính",gender:"Nữ"});             
             if(item.chandoan_out_main_icd10)
                 arrayInfo.push({id:"4",name:"ICD",value:item.chandoan_out_main_icd10});
             if(item.chandoan_out_main)
                 arrayInfo.push({id:"5",name:"Mã bệnh",value:item.chandoan_out_main});                
